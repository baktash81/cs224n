...
def naiveSoftmaxLossAndGradient(
    ...
    # y = np.transpose(outsideVectors)[outsideWordIdx]

    gradCenterVec = (yHat - y) @  outsideVectors

    gradOutsideVecs = np.outer(np.transpose(yHat - y), centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    part1 = -np.log(sigmoid(uO @ centerWordVec))
    part2 = np.log(sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec))
    loss = part1 - part2.sum()

    cpart1 = (sigmoid(uO @ centerWordVec) - 1) * uO
    cpart2 = (1 - sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec)) @ outsideVectors[negSampleWordIndices]
    gradCenterVec = cpart1 + cpart2

    opart1 = np.zeros_like(outsideVectors)
    opart1[outsideWordIdx, :] = (sigmoid(outsideVectors[outsideWordIdx] @ centerWordVec) - 1) * centerWordVec
    unique, indices, counts = np.unique(negSampleWordIndices, return_index=True, return_counts=True)
    samples = sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec)[indices]
    opart1[unique, :] = -1 * ((samples - 1)* counts)[:,None] * centerWordVec[None,:]
    gradOutsideVecs = opart1
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords :
        raw_loss,raw_gradCenterVecs,raw_gradOutsideVectors = word2vecLossAndGradient(centerWordVectors[centerIndex],word2Ind[outsideWord],outsideVectors,dataset)
        loss += raw_loss
        gradCenterVecs[centerIndex] += raw_gradCenterVecs
        gradOutsideVectors += raw_gradOutsideVectors
