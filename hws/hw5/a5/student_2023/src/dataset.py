...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prefix_length = random.randint(0, trunc_length - trunc_length // 4)
        suffix_length = trunc_length - prefix_length - trunc_length // 4

        prefix = document[:prefix_length]
        masked_content = document[prefix_length:prefix_length + trunc_length // 4]
        suffix = document[prefix_length + trunc_length // 4:prefix_length + trunc_length // 4 + suffix_length]
        pads = self.PAD_CHAR * (self.block_size - len(prefix) - len(suffix) - 2 * len(self.MASK_CHAR) - len(masked_content))
        masked_string = f"{prefix}{self.MASK_CHAR}{suffix}{self.MASK_CHAR}{masked_content}{pads}"
