...
if args.function == 'pretrain':
    ...
    Hyperparams = trainer.TrainerConfig(
        max_epochs = 650,
        batch_size = 128,
        learning_rate = args.pretrain_lr,
        lr_decay = True,
    ...
    trainer.Trainer(model, pretrain_dataset, None, Hyperparams).train()
    ...
    fineTune = dataset.NameDataset( pretrain_dataset, open(args.finetune_corpus_path).read() )
    ...
    if args.eval_corpus_path is not None:
        eval = dataset.NameDataset(pretrain_dataset, open(args.eval_corpus_path).read())
    else:
        eval = None

    trainer.Trainer(model, fineTune, eval, Hyperparams).train()
