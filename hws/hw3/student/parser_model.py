...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # first added layer : 
        self.hidden_to_hidden2_weight = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size))
        nn.init.xavier_uniform_(self.hidden_to_hidden2_weight)
        
        self.hidden_to_hidden2_bias = nn.Parameter(torch.Tensor(self.hidden_size))
        nn.init.uniform_(self.hidden_to_hidden2_bias)


        # seccond added layer :
        self.hidden2_to_hidden3_weight = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size))
        nn.init.xavier_uniform_(self.hidden2_to_hidden3_weight)
        
        self.hidden2_to_hidden3_bias = nn.Parameter(torch.Tensor(self.hidden_size))
        nn.init.uniform_(self.hidden2_to_hidden3_bias)
    ...
    def forward(self, w):
        ...
        ## h = ReLU(xW + b1)
        ## l = hU + b2
        # use weight and bias from init function that we have declared
        relu = nn.ReLU()

        #first layer : 
        h = relu(torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias)

        h = self.dropout(h)

        # first added layer :
        h2 = relu(torch.matmul(h, self.hidden_to_hidden2_weight) + self.hidden_to_hidden2_bias)
        h2 = self.dropout(h2)

        #seccond added layer : 
        h3 = relu(torch.matmul(h2, self.hidden2_to_hidden3_weight) + self.hidden2_to_hidden3_bias)
        h3 = self.dropout(h3)

        # last layer :
        logits = torch.matmul(h3, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
